...
<template>
  ...
  <div class="flex justify-between w-[90%] mx-auto">
    ...
    <UButton
      ...
      class="text-xs px-2 py-0 roundex-3xl bg-[rgba(134,144,158,0.7)] dark:bg-white"
      >Logout</UButton
    >
  </div>
  ...
  <section
    v-if="displayedUser"
    class="flex pt-7 gap-3 justify-between w-[90%] mx-auto max-w-xl"
  >
    ...
    <div class="flex items-end mr-2">
      <UButton
        ...
        class="text-xs px-1 py-1 roundex-3xl bg-[rgba(45,97,175,0.7)] dark:text-white"
  ...
  <div v-if="!editProfile">
      ...
      <div>
        <div class="flex justify-between w-[90%] mx-auto">
          <div class="flex gap-1 items-center w-[90%] mx-auto">
            ...
            <UBadge
              :label="`Favourite cards (${fetchedFavouriteCards.length})`"
              class="px-0 w-full text-sm bg-transparent text-[#A25772] font-bold dark:text-white dark:bg-[#202020]"
            />
          </div>
          <div class="w-2/6 flex justify-end">
            <NuxtLink :to="`/favourites`">
              <UButton
                label="Show more"
                class="border shadow-none border-transparent bg-transparent text-[rgba(0,0,0,0.8)] px-0 py-0 text-sm font-bold dark:text-white dark:bg-[#202020]"
              />
            </NuxtLink>
          </div>
        </div>
        <div
          v-if="fetchedFavouriteCards.length > 0"
          class="rounded-xl border border-blue-200 w-[90%] mx-auto flex gap-5 pl-5 pr-2 items-center overflow-scroll overflow-y-hidden bg-blue-100 dark:text-white dark:bg-[#202020]"
        >
          <div
            v-for="fav in fetchedFavouriteCards.slice(0, 10)"
            :key="fav"
            class="w-fit py-7"
          >
            <NuxtLink :to="`card/${fav.card}`">
              <NuxtImg
                :src="fav.image"
                class="rounded-sm max-w-[150px] h-[100px] w-[64px] lg:h-[160px] lg:w-[108px]"
              />
            </NuxtLink>
          </div>
          <NuxtLink :to="`/favourites`">
            <UButton
              v-if="fetchedFavouriteCards.length > 10"
              icon="i-heroicons-forward"
              class="border shadow-none border-transparent bg-transparent text-[rgba(0,0,0,0.8)] px-0 py-0 text-s font-bold"
            />
          </NuxtLink>
        </div>
        ...
        <div
          v-if="fetchedFavouriteCards.length == 0"
          class="rounded-xl border border-blue-200 w-[90%] mx-auto min-h-[170px] flex flex-col text-[rgba(0,0,0,0.5)] gap-1 pl-5 pr-2 items-center justify-center bg-blue-100 dark:text-white dark:bg-[#202020]"
        >
    ...
    <section class="pt-8 w-[90%] mx-auto max-w-xl">
      <div class="flex justify-between w-[90%] mx-auto">
        <div class="flex gap-1 items-center">
          ...
          <UBadge
            :label="`Friends (${friendUsers.length})`"
            class="px-0 text-sm bg-transparent text-[#A25772] font-bold dark:text-white dark:bg-[#202020]"
          />
        </div>
        <div>
          <UButton
      ...
      <div
        v-if="friendUsers.length > 0"
        class="rounded-xl border border-blue-200 w-[90%] mx-auto flex items-center h-44 bg-blue-100 overflow-scroll overflow-x-hidden dark:text-white dark:bg-[#202020]"
      >
      ...
      <div
        v-if="friendUsers.length === 0"
        class="rounded-xl border border-blue-200 w-[90%] mx-auto flex flex-col justify-center items-center h-44 bg-blue-100 text-[rgba(0,0,0,0.5)] dark:text-white dark:bg-[#202020]"
      >
    ...
    <section class="py-8 w-[90%] mx-auto max-w-xl">
      <div class="flex gap-1 items-center w-[90%] mx-auto">
        ...
        <UBadge
          :label="`Decks (${decks.length})`"
          class="px-0 text-sm bg-transparent text-[#A25772] font-bold dark:text-white dark:bg-[#202020]"
        />
      </div>
      <div
        v-if="decks.length > 0"
        class="rounded-xl border border-blue-200 w-[90%] mx-auto flex gap-5 pl-5 pr-2 items-center h-44 bg-blue-100 dark:text-white dark:bg-[#202020]"
      ></div>
      <div
        v-if="decks.length == 0"
        class="rounded-xl border border-blue-200 w-[90%] mx-auto min-h-[170px] flex flex-col text-[rgba(0,0,0,0.5)] gap-1 pl-5 pr-2 items-center justify-center bg-blue-100 dark:text-white dark:bg-[#202020]"
      >
        <span class="text-center"
          >Meet the deck on a coffee break soon to host</span
        >
